...
export const getFilteredTasks = (params: IGetFilteredTasks) => {
	const { tasks, searchText, filters } = params;
	...
	return tasks?.filter((task) => {
		...
		if (
			...
			!task.assignees.some((user) => filters.assignees.includes(user.id))
		) {
			return false;
		}
		...
		if (filters.dateRange) {
			...
			if (taskStartDate && taskEndDate) {
				...
				return (
					taskEndDate.isBefore(endDate) || taskEndDate.isSame(endDate)
				);
			} else {
				return false;
			}
